...
void byte_array::writeString(const std::string &str) {
	size_t str_len = str.size();
	writeShort(str_len);
	memcpy(data_ptr, str.data(), str_len);
	data_ptr += str_len;
	data_len += str_len;
}

std::string byte_array::readString() {
	int str_len = readShort();
	std::string str(str_len, '\0');
	strncpy(&str[0], reinterpret_cast<const char *>(data_ptr), str_len);
	data_ptr += str_len;
	return str;
}
